...
```{r gsea-ko-effect}
ko_gsea = gsea_cp(ko, "knockout-effect")
ko_gsea_res = convert_gsea_ids(ko_gsea$summary, entrezsymbol)
```
...
```{r gsea-wt-treatment-effect}
treatwt_gsea = gsea_cp(ko, "treatment-effect-on-wt")
treatwt_gsea_res = convert_gsea_ids(treatwt_gsea$summary, entrezsymbol)
```
...
```{r gsea-treatment-on-ko}
treatko_gsea = gsea_cp(ko, "treatment-effect-on-ko")
treatko_gsea_res = convert_gsea_ids(treatko_gsea$summary, entrezsymbol)
```
...
```{r treatko-specific-gsea}
treatkospecific_gsea = gsea_cp(treatkospecific, "treatment-effect-on-kospecific")
treatkospecific_gsea_res = convert_gsea_ids(treatkospecific_gsea$summary, entrezsymbol)
```
...
# Marker genes
Here we try to find genes that are markers for each of the four groups. By
markers we mean genes that are up or down only in that specific group compared
to all of the other groups. Since we only have four groups, we can do this by
looking, for each group, at all pairwise comparisons with the other groups, and
then picking out the genes which are significantly up or down in a given
group compared to each of the other groups. The heatmaps are of the logged,
normalized counts for these genes in each sample.

To simplify the output, we took those genes and did a differential expression
test of the gene in the given sample group against the average of all the
other samples. We didn't use this to select the genes, it is just to have an
idea of which direction and an overall estimate of the magnitude of the change
in gene expression in the particular group. These are available at the bottom
in the section `Marker gene tables`
...
allup = (comp1[genes, "log2FoldChange"] > 0) & (comp2[genes, "log2FoldChange"] > 0) & (comp3[genes, "log2FoldChange"] > 0)
alldown = (comp1[genes, "log2FoldChange"] < 0) & (comp2[genes, "log2FoldChange"] < 0) & (comp3[genes, "log2FoldChange"] < 0)
samedir = allup | alldown
wtmarkers = genes[samedir]
heatmap_fn(log(ncounts[wtmarkers,] + 1))
combined = results(dds, contrast=list(c("gtwt_untreated"),
        c("gtwt_ccl4", "gtyaptaz_ccl4", "gtyaptaz_untreated")), listValues=c(1,-1/3))
combined = combined[wtmarkers,]
write_deseq(combined, "wt-untreated-markers.csv")
```

## WT, CCL4 treated
```{r ccl4-untreated-markers}
...
combined = results(dds, contrast=list(c("gtwt_ccl4"),
        c("gtwt_untreated", "gtyaptaz_ccl4", "gtyaptaz_untreated")),
        listValues=c(1,-1/3))
...
combined = results(dds, contrast=list(c("gtyaptaz_untreated"),
        c("gtwt_untreated", "gtyaptaz_ccl4", "gtwt_ccl4")),
        listValues=c(1,-1/3))
...
genes = intersect(genes, rownames(subset(comp2, padj < 0.05 & abs(log2FoldChange) > 3)))
genes = intersect(genes, rownames(subset(comp3, padj < 0.05 & abs(log2FoldChange) > 3)))
allup = (comp1[genes, "log2FoldChange"] > 0) & (comp2[genes, "log2FoldChange"] > 0) & (comp3[genes, "log2FoldChange"] > 0)
alldown = (comp1[genes, "log2FoldChange"] < 0) & (comp2[genes, "log2FoldChange"] < 0) & (comp3[genes, "log2FoldChange"] < 0)
samedir = allup | alldown
yaptaztreatedmarkers = genes[samedir]
heatmap_fn(log(ncounts[yaptaztreatedmarkers,] + 1), fontsize=8)
combined = results(dds, contrast=list(c("gtyaptaz_ccl4"),
        c("gtwt_untreated", "gtyaptaz_untreated", "gtwt_ccl4")),
        listValues=c(1,-1/3))
...
## Marker gene tables
[WT, untreated markers](wt-untreated-markers.csv)

[WT, CCL4 markers](wt-ccl4-markers.csv)

[YapTaz, untreated markers](yaptaz-untreated-markers.csv)

[YapTaz, CCL4 markers](yaptaz-ccl4-markers.csv)


# Pairwise-comparisons
Here we look at the data in a slightly different way. Instead of looking at
interactions of genotype and treatment, we do pairwise comparisons between
the groups. This is a lot easier to think about, but makes the question
question "which genes are specifically affected by treatment in the KO" a lot
harder to answer.

## Differential expression
### KO untreated vs WT untreated
This compares the KO untreated samples to the WT untreated samples. Here,
positive log2 fold changes

```{r pairwise-ko-effect}
ko = results(dds, contrast=c("gt", "yaptaz_untreated", "wt_untreated"),
             addMLE=TRUE)
komarked = markup_deseq(ko)
plotMA_full(ko)
volcano(ko)
...
```
There are `r nrow(subset(ko, padj < 0.05))` genes different in the untreated KO
samples compared to the untreated WT samples.


### WT treated vs WT untreated
```{r pairwise-wt-treatment-effect}
...
There are `r nrow(subset(treatwt, padj < 0.05))` genes different in the WT
treated samples compared to the WT untreated samples.


### KO treated vs KO untreated
```{r pairwise-treatment-on-ko}
...
```
There are `r nrow(subset(treatko, padj < 0.05))` genes different in the KO
treated samples compared to the KO untreated samples.

### Differential expression output tables

[KO untreated vs WT untreated](pairwise-ko-untreated-vs-wt-untreated.csv)

[WT treated vs WT untreated](pairwise-wt-treated-vs-wt-untreated.csv)

[KO treated vs KO untreated](pairwise-ko-treated-vs-ko-untreated.csv)

### Venn diagram

Here is a Venn diagram of the overlaps of genes called significant in each
of these comparisons.

```{r venn-diagram-of-overlaps}
kosig = subset(ko, padj < 0.05)
treatwtsig = subset(treatwt, padj < 0.05)
treatkosig = subset(treatko, padj < 0.05)
library(gplots)
venn(list("WT treated vs WT untreated"=rownames(treatwtsig),
          "KO untreated vs WT untreated"=rownames(kosig),
          "KO treated vs KO untreated"=rownames(treatkosig)))
```

# GSEA
Here we ran GSEA on the pairwise comparisons, we could also run these on
the interaction model if that would be helpful.

## Pairwise comparisons {.tabset}

### WT untreated vs KO untreated

```{r pairwise-ko-effect-gsea}
pairwise_ko_gsea = gsea_cp(ko, "pairwise-ko-effect-gsea")
pairwise_ko_gsea_res = convert_gsea_ids(pairwise_ko_gsea$summary, entrezsymbol)
```

```{r write-pairwise-knockout-effect-gsea}
pairwise_ko_gsea_res = pairwise_ko_gsea_res[order(pairwise_ko_gsea_res$p.adjust),]
write_csv(pairwise_ko_gsea_res, "pairwise-ko-gsea.csv")
```

There are
`r nrow(subset(pairwise_ko_gsea_res, p.adjust < 0.05))` enriched pathways via
GSEA.

### WT treated vs WT untreated

```{r pairwise-wt-treatment-effect-gsea}
pairwise_treatwt_gsea = gsea_cp(treatwt, "pairwise-treatwt-effect-gsea")
pairwise_treatwt_gsea_res = convert_gsea_ids(pairwise_treatwt_gsea$summary,
                                             entrezsymbol)
```

```{r write-pairwise-gsea-treatment}
pairwise_treatwt_gsea_res = pairwise_treatwt_gsea_res[order(pairwise_treatwt_gsea_res$p.adjust),]
write_csv(pairwise_treatwt_gsea_res, "pairwise-treatwt-gsea.csv")
```


There are `r nrow(subset(pairwise_treatwt_gsea_res, p.adjust < 0.05))` enriched
pathways via GSEA.

### KO treated vs KO untreated

```{r pairwise-ko-treatment-gsea}
pairwise_treatko_gsea = gsea_cp(treatko, "pairwise-treatko-effect-gsea")
pairwise_treatko_gsea_res = convert_gsea_ids(pairwise_treatko_gsea$summary,
                                             entrezsymbol)
```

```{r write-gsea-pathways}
pairwise_treatko_gsea_res = pairwise_treatko_gsea_res[order(pairwise_treatko_gsea_res$p.adjust),]
write_csv(pairwise_treatko_gsea_res, "pairwise-treatko-gsea.csv")
```

There are `r nrow(subset(pairwise_treatko_gsea_res, p.adjust < 0.05))` enriched
pathways via GSEA.

## GSEA output tables

[WT untreatred vs KO untreated](pairwise-ko-gsea.csv)

[WT treated vs WT untreated](pairwise-treatwt-gsea.csv)

[KO untreated vs KO treated](pairwise-treatko-gsea.csv)

## KO treated vs KO untreated GSEA plots {.tabset}

There aren't any pathways that are significantly different in the treatment
alone and the knockout alone. Here we make plots for the top 10 pathways by
adjusted p.value for the KO treated vs KO untreated comparison, so the effect of
treatment on the kncokout. kThese turned out to all be KEGG pathways, so we'll
include a view of the pathway as well for each.

```{r tryplots, include=FALSE}
library(imager)
library(pathview)
library(XML)
top10 = head(pairwise_treatko_gsea_res$ID, 10)
sn = signal_to_noise(treatko)
plots = NULL
for(result in top10) {
  plots = c(plots, knitr::knit_child('gseaplot.Rmd'))
}
```

`r paste(plots, collapse = '\n')`


## Pathway summary
We aren't finding much that is affected just by treatment or just by the
knockout. We find significant differences between the treated KO and the
untreated KO, however. This indicates that the neither the knockout nor the
treatment by themselves are perturbing the transcriptional state of the cells.
The treatment seems to specifically affect the KO cells. We found something
similar when we looked at this from the interaction point of view as well.


## DE enrichment pathway analysis
Here we repeat the pathway analysis, but using the three sets of genes
identified by the pairwise interactions as input.

### KO untreated vs WT untreated
```{r pairwise-pathway-ko-effect}
library(readr)
ko_enrich = enrich_cp(ko, "knockout-effect")
ko_out = convert_core_ids(subset(ko_enrich$summary, qvalue < 0.05))
ko_out = direction_breakdown(ko_out, ko)
...
### WT treated vs WT untreated
```{r pairwise-wt-treatment-effect-pathway}
treatwt_enrich = enrich_cp(treatwt, "treatment-effect-on-wt")
treatwt_out = convert_core_ids(subset(treatwt_enrich$summary, qvalue < 0.05))
treatwt_out = direction_breakdown(treatwt_out, treatwt)
treatwt_int = read_csv("followup/treatment-wt.txt", col_names="ID")
...
### KO treated vs KO untreated
```{r pairwise-ko-treatment-effect-pathway}
treatko_enrich = enrich_cp(treatko, "treatment-effect-on-ko")
treatko_out = convert_core_ids(subset(treatko_enrich$summary, qvalue < 0.05))
treatko_out = direction_breakdown(treatko_out, treatko)
treatko_int = read_csv("followup/treatment-ko.txt", col_names="ID")
...
### Pathway analysis output tables
[KO untreated vs WT untreated, pathways](pairwise-ko-untreated-vs-wt-untreated-pathways.csv)

[WT treated vs WT untreated, pathways](pairwise-wt-treated-vs-wt-untreated-pathways.csv)

[KO treated vs KO untreated, pathways](pairwise-ko-treated-vs-ko-untreated-pathways.csv)

# Comparison of interaction and pairwise
## Knockout effect
Here we compare the effect of the knockout identified by doing the pairwise
comparisons and the effect identified by looking at the
"ko" term in the interaction model:

```{r pairwise-comp-ko}
pairwise = read_csv("pairwise-ko-untreated-vs-wt-untreated.csv")
interaction = read_csv("knockout-effect.csv")
combined = pairwise %>%
  left_join(interaction, by="id") %>%
  dplyr::select(id, log2FoldChange.y, lfcMLE)
colnames(combined) = c("id", "interaction", "paired")
ggplot(combined, aes(interaction, paired)) + geom_point()
```

## WT treatment effect
Here we compare the effect of the treatment on the wildtype that we
identified by doing the pairwise comparisons and the effect identified
by looking at the "treatment" term in the interaction model:

```{r pairwise-wt-treatment}
...
## KO treatment effect
Here we compare the effect of the treatment on the knockout that we
identified by doing the pairwise comparisons of the KO treated to the KO
untreated and the effect identified by looking at the combination of the
main treatment effect and the treatment effect specific to the KO.
